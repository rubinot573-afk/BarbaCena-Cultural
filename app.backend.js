...
async function fetchContent() {
  try {
    ...
    console.error(error);
    if (ui.newsList) {
      ui.newsList.innerHTML = '<p>Não foi possível carregar os dados do servidor. Rodando offline.</p>';
    }
  }
}
...
function addEventListeners() {
  ...
  ui.newsForm.addEventListener('submit', async (event) => {
    ...
    try {
      if (index === '') {
        await postData('/api/content/news', { title, content, image, date: new Date().toLocaleDateString('pt-BR') });
      } else {
        await putData(`/api/content/news/${index}`, { title, content, image });
      }
      await fetchContent();
      renderAdminLists();
  ...
  ui.galleryForm.addEventListener('submit', async (event) => {
    ...
    try {
      if (index === '') {
        await postData('/api/content/gallery', { title, image, description });
      } else {
        await putData(`/api/content/gallery/${index}`, { title, image, description });
      }
      await fetchContent();
      renderAdminLists();
